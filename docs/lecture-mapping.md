...
| §1.4 Constructing for verification             | PHPUnit suites + `/healthz` endpoint                                     |
...
| §1.5 Reuse                                     | PSR-4 autoload + Docker base images + Composer / pip pins                |
| §1.6 Standards in construction                 | PSR-12 (`phpcs.xml`) + `.editorconfig` + commit format in `CONTRIBUTING.md` |
...
| §2.2.3 Perfective change                       | Feature request template                                                 |
| §2.2.4 Preventive change                       | `chore/*` branches, `phpcs`, CodeQL                                      |
...
| §3.4.4 Defensive programming                   | `JSON_THROW_ON_ERROR`, prepared statements, input validation             |
| §3.4.5 Error handling                          | Try / catch in router maps `InvalidArgumentException` → 422              |
| §3.4.12 Distributed software                   | nginx + php-fpm + mysql in separate containers, talking over the bridge  |
